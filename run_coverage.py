...
if __name__ == '__main__':
    if 'TRAVIS' in os.environ and os.environ.get('TRAVIS_PYTHON_VERSION', 'unknown-version') == '3.5':
        rc = Popen(['coverage', 'run', '--source=prestans3', '--rcfile', os.path.expandvars('${envdir}/.coveragerc'),
                    'python', os.path.expandvars('${toxinidir}/setup.py'), 'test'], stdout=PIPE, stderr=STDOUT)
        while True:
